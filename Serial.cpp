...
int main() {
    ...
    // Serial AES Encryption with larger input size
    auto start = std::chrono::high_resolution_clock::now();
    AES_encrypt_decrypt_serial(input, output, &aes_key, dataSize);
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end - start;
    std::cout << "AES Serial Time (1MB): " << duration.count() << " seconds\n";

    // Serial DES Encryption with larger input size
    start = std::chrono::high_resolution_clock::now();
    DES_encrypt_decrypt_serial(input, output, &des_schedule, dataSize);
    end = std::chrono::high_resolution_clock::now();
    duration = end - start;
    std::cout << "DES Serial Time (1MB): " << duration.count() << " seconds\n";
